...
class RKNCell(nn.Module):
    ...
    def get_transition_model(self, post_mean: torch.Tensor) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        coefficients = torch.nan_to_num(coefficients, nan=0)
    ...
    def _update(self, prior_mean: torch.Tensor, prior_cov: Iterable[torch.Tensor],
        ...
        max_value=torch.max(denominator)
        q_upper = cov_u * denominator/max_value
        q_lower = cov_s * denominator/max_value
        # q_upper = torch.nan_to_num(q_upper, nan=0)
        # q_lower = torch.nan_to_num(q_lower, nan=0)
